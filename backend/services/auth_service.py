...
class AuthorizationService:
    ...
    def register(self, username, password) -> dict:
        ...
        if not self._hasher.is_strong(password):
            raise PasswordError("не надежный пароль [REGISTER]")
    ...
    def login(self, username, password) -> dict:
        username = username.strip()
        if not self._user_repo.exists_user(username):
            raise AuthError(f'User {username} не существует [LOGIN]')
        user = self._user_repo.get_user_by_username(username)
        self._hasher.verify(password, self._hasher.hash(password))
        session = self._session_repo.create_session(user.id)
